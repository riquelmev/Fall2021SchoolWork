...
def euclidean_graph():
    ...
    for edge in g.edges():
        g[edge[0]][edge[1]]['weight'] = sqrt((x[edge[0]]-x[edge[1]])**2 + (y[edge[0]]-y[edge[1]])**2)
...
def main():
    ...
    best_path_ants, best_weight_ants, evol_ants = simulation_ants(iterations_ants)
    print("Running Genetic")
    best_path_gen, best_weight_gen, evol_gen = simulation_genetic(iterations_genetic)
    plt.plot(evol_ants)
    ...
    print("Our algorithm ran for " + str(iterations_ants) + " iterations.")
    print("The 3/2 Hamilton approximation path using the Christofides algorithm is: ", sol ," with a path weight of:",path_weight(sol))
    print("The best solution our genetic algorithm could produce is: ", best_path_gen, "with a path weight of:", best_weight_gen)
    print("Is our solution valid?" ,isValid(best_path_gen))
    print("The best solution our Ant Colony algorithm could produce is: ", best_path_ants, "with a path weight of:", best_weight_ants)
