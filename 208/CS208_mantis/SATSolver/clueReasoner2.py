...
def initialClauses():
    ...
    for c in cards:
        for p1 in extendedPlayers:
            for p2 in extendedPlayers:
                if p1 != p2:
                    clauses.append([-1 * getPairNumFromNames(p1,c), -1 * getPairNumFromNames(p2,c)])
    ...
    for category in [suspects, weapons, rooms]:
        for c1 in category:
            for c2 in category:
                if c1 != c2:
                    clauses.append([-1 * getPairNumFromNames(caseFile,c1), -1 * getPairNumFromNames(caseFile,c2)])
    return clauses
