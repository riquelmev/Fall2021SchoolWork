...
class BlackjackMDP(util.MDP):
    ...
    def succAndProbReward(self, state: State, action: str) -> List[PossibleResult]:
        ...
        for i in range(len(self.cardValues)):
            sum += state.deckCounts[i]
        possibilityList = []
        if action == 'Peek':
            if state.nextCard == None:
                for i in range(len(self.cardValues)):
                    prob = state.deckCounts[i]/ sum
                    if prob != 0:
                        newState = State(handTotal=state.handTotal,
                                    nextCard=self.cardValues[i],
                                    deckCounts=State.deckCounts)
                        possibilityList.append(PossibleResult(successor=newState,
                                    probability=prob,
                                    reward=-1 * self.peekCost))
            else:
                possibilityList.append(PossibleResult(successor=state,
                                   probability=1,
                                   reward=-1 * self.peekCost))
        if action == 'Take':
            if state.nextCard == None:
                for i in range(len(self.cardValues)):
                    prob = state.deckCounts[i]/ sum
                    newTotal = state.handTotal + self.cardValues[i]
                ...
                newState = State(handTotal = newTotal,
                                    nextCard=None,
                                    deckCounts=newDeckCounts)
                possibilityList.append(PossibleResult(successor=newState,
                                    probability=1,
