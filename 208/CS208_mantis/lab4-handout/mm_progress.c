...
int mm_init(void) {
    ...
    head = heap_start; /* declare global variable to store head of explicit free list */
...
static void printLen(){
    ...
    for (char *cur_block = (void *) GET(NXTP(head)); cur_block == NULL; cur_block = (void *) GET(NXTP(cur_block))) {
        fprintf(f,"size = %zu \n", GET_SIZE(HDRP(cur_block)));
        len ++;
    }
...
static void insertFree(void *bp){
    PUT(NXTP(bp), GET(NXTP(head)));
    PUT(PRVP(bp), (size_t) head);
    PUT(NXTP(head), (size_t) bp);
}
...
static void removeFree(void *bp){
    if (PRVP(bp) == NULL) {
        ...
        PUT(NXTP(GET(PRVP(bp))) , GET(NXTP(bp)));
        PUT(PRVP(GET(NXTP(bp))) , GET(PRVP(bp)));
    }
}
